...
# Oracle Status - Show Done Oracle verification status for all tasks in an epic
# Usage: /pm:oracle-status <epic_name>
...
if [ -z "$epic_name" ]; then
    ...
    echo "Usage: /pm:oracle-status <epic_name>"
    exit 1
fi
...
# Scan All Tasks
echo "═══════════════════════════════════════════════════════"
echo "📊 Done Oracle Status for Epic: $epic_name"
echo "═══════════════════════════════════════════════════════"
echo ""
...
for task_file in "$epic_dir"/*.md; do
    if [[ "$(basename "$task_file")" =~ ^[0-9]+\.md$ ]]; then
        ...
        if [ "$verdict" = "DONE" ] && [ "$done_policy_met" = "True" ]; then
            ...
            echo "   Oracle Status: ❓ ERROR/INCONCLUSIVE ($verdict, confidence: $confidence, tier: $tier)"
